(function () {
  ...
  document.addEventListener("keydown", (event) => {
    if (event.target.matches("input") && event.target !== mobileInput) {
      return;
    }
    if (!profileModal.classList.contains("hidden") || !resultModal.classList.contains("hidden") || !aboutModal.classList.contains("hidden")) {
      return;
  ...
  mobileInput.addEventListener("input", () => {
    const letters = mobileInput.value.toLowerCase().replace(/[^a-z]/g, "");
    mobileInput.value = "";
    [...letters].forEach((letter) => handleInput(letter));
  });
